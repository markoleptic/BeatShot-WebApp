        ...
        <div className="flex-container-row">
          ...
          <article className="flex-container-column" id="article">
            <p>
              ...
              uses in its target spawning system. The code blocks are supplementary and I follow a general pattern when inserting them:
              introduce the function and context, insert code block, describe what&#39;s happening in the code block.
            </p>
            <div className="article-section" ref={Ref_Classes} id="classes-header">
              <BlogHeading headingText="Classes" headingLevel={1} />
              <p>
                This section gives a brief description of all classes involved in BeatShot&#39;s target spawning system and establishes the
                context needed for the next section.
              </p>
              <div className="article-subsection" ref={Ref_USpawnArea} id="classes-USpawnArea">
                ...
                <p>
                  ...
                  total spawn area dimensions are the Horizontal and Vertical Spread from custom game mode options. These dimensions
                  correspond to the actual size in Unreal units, and when maxed out represent 3.2 million individual points.
                </p>
                <p>
                  I needed to find a way to keep track of target locations and the total area that they occupied. Iterating through 3.2
                  million points isn&#39;t even an option, so the total two-dimensional spawn rectangle is broken into the smaller
                  rectangles, where each one is represented by a SpawnArea.
                </p>
                <ul>
                  ...
                  <li>
                    <FontAwesomeIcon icon={faCrosshairs} className="li-icon" />
                    info about the targets that have spawned within it, such as the scale the target spawned with and its global unique
                    identifier
                  </li>
                  <li>
                    <FontAwesomeIcon icon={faCrosshairs} className="li-icon" />
                    flags that indicate if a target in its area is activated, deactivated, or if its only holding the location (target is no
                    longer visible, but still not allowed to spawn there)
                  </li>
                </ul>
              </div>
              <div className="article-subsection" ref={Ref_ATargetManager} id="classes-ATargetManager">
                ...
                <p>
                  ...
                  <em>Target Activation Responses</em>, which are used to describe what a target does when it is activated. Some examples
                ...
                <div className="article-subsection-2" ref={Ref_UBoxComponent} id="classes-UBoxComponent">
                  ...
                  />
                  <p>
                    The Target Manager uses seven box components that can all be considerd 2-D planes since one out of three dimensions is
                    always zero.
                  </p>
                  <p>
                    The first plane (middle red in Figure 1) corresponds to the total spawn area, or the 2-D area encompassed by all
                    SpawnArea objects.
                  </p>
                  <p>
                    The other six form a rectangular prism (blue, green, front & back red planes in Figure 1) used to confine moving
                    targets. I would&#39;ve preferred to only use one box component, but &#34;hollow collision&#34; is not a thing. They
                    form a closed volume, but are offset in Figure 1 for visiblity.
                  </p>
                  <figure>
                ...
                <div className="article-subsection-2" ref={Ref_UReinforcementLearningComponent} id="classes-UReinforcementLearningComponent">
                  ...
                  <p>
                    It uses the Q-Learning algorithm to find the target spawn location that the player is least likely to successfully
                    destroy based on past performance.
                  </p>
                  <p>
                    The component uses a 2-D array where each element in the array represents the reward from having spawned a target at
                    location A and immediately spawning a target at location B. A simpler approach would be to only consider the reward for
                    spawning a target a location B, but using the previous spawn location generates better predictions, even though it takes
                    longer to train the model. It always scales the total spawn area down to a 5x5 grid, meaning that the 2-D array will
                ...
                <div className="article-subsection-2" ref={Ref_USpawnAreaManagerComponent} id="classes-USpawnAreaManagerComponent">
                  ...
                  <p>
                    It calculates the size that all SpawnAreas are set to, with the main limiting factor being that RL Component is always a
                    5x5 grid. This means that the number of horizontal and vertical SpawnAreas must also be in multiples of five, though
                    they don&#39;t need to be the same multiple of five.
                  </p>
                  <p>
                    This component used to be part of the base Target Manager class, but as the class grew in size, it made sense to
                    encapsulate all SpawnArea-related functions into a component.
                  </p>
                </div>
              </div>
              <div className="article-subsection" ref={Ref_ATarget} id="classes-ATarget">
                ...
                <p>
                  ...
                  handling <em>Target Deactivation Responses</em>, which are used to describe what a target does when it is deactivated.
                  Some examples include applying immunity and playing effects. To trigger a Target Deactivation Response, a{" "}
                  <em>Target Deactivation Condition</em> must be met. In addition to deactivation, the target is also responsible for its
                  own destruction, which is triggered by a <em>Target Destruction Condition</em>. Deactivation and Destruction are discussed
                ...
                <div className="article-subsection-2" ref={Ref_UProjectileMovementComponent} id="classes-UProjectileMovementComponent">
                  ...
                  <p>
                    ...
                    <BSInlineFunction className={"ATarget"} functionName={"SetTargetDirection"} /> when it wants to change the velocity or
                    direction. These also happen to be Target Activation Responses.
                  </p>
                  <p>
                ...
                <div className="article-subsection-2" ref={Ref_UBSAbilitySystemComponent} id="classes-UBSAbilitySystemComponent">
                  ...
                  <p>
                    ...
                    System Component (ASC), enabling communication between their ASC and the target ASC.
                  </p>
                  <p>
                    The ASC enables us to apply immunity to the target for the different damage types (and control them independently) and
                    has built in support for tracking attributes like health.
                  </p>
                  <p>
                    Sidenote: The ASC comes from the Gameplay Ability System plugin (GAS) by Unreal. I might do a seperate article covering
                ...
                <div className="article-subsection-2" ref={Ref_UBSHealthComponent} id="classes-UBSHealthComponent">
                  ...
                  <p>
                    ...
                    attribute of the target. The target binds the health component&#39;s OnHealthChanged delegate to its{" "}
                  ...
                  <p>
                    With this configuration, the target knows if it was the actor who caused the damage event. This is important becuase the
                    target damages itself if its Max Lifespan expires.
                  </p>
                </div>
              </div>
              <div className="article-subsection" ref={Ref_ABSGameMode} id="classes-ABSGameMode">
                ...
                <p>
                  ...
                  analyzer and notifies the Target Manager each time a beat threshold is met.
                </p>
              </div>
            </div>
            <div className="article-section" ref={Ref_TargetLifeCycle} id="target-lifecycle">
              ...
              <p>
                Every target goes through the same lifecycle of spawning, activation, deactivation, and destruction. Before discussing those
                events, I&#39;ll describe the initialization process that the involved actors go through.
              </p>
              <div className="article-subsection" ref={Ref_Initialization} id="target-lifecycle-Initialization">
                ...
                <p>
                  When you first load into the map, the game mode spawns the Target Manager and passes the game mode configuration to it. It
                  then passes the relevant configuration settings to each of its components inside{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"Init"} />. More specifically, it does the following:
                </p>
                <ol>
                  <li>Sets the dimensions of all box components</li>
                  <li>
                    Calls <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"Init"} />
                  </li>
                  <li>
                    Calls <BSInlineFunction className={"UReinforcementLearningComponent"} functionName={"Init"} />
                  </li>
                </ol>
                <p>
                  After receiving the game mode config from the Target Manager,{" "}
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"InitializeSpawnAreas"} /> is called to generate
                  all SpawnAreas for the game mode. This is where the SpawnArea Manager Component determines how many SpawnAreas the total
                  spawn area needs to be divided into. This happens inside{" "}
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"SetSpawnMemoryValues"} />. Once the width and
                  height of the SpawnAreas are found, the SpawnArea Manager Component creates a new USpawnArea object for each section of
                  the total spawn area. It then calls <BSInlineFunction className={"USpawnArea"} functionName={"Init"} />, which sets up
                  basic information about the SpawnArea. Notably, it also specifies which type of section inside the total spawn area it
                  belongs to, represented by EGridIndexType:
                </p>
                <BSCodeBlock code={EGridIndexType} language={"c"} showLineNumbers={false} fontSize="0.65rem" />
                <p>
                  The SpawnArea then finds all adjacent SpawnAreas based on the Grid Index Type and the number of horizontal SpawnAreas that
                  make up the total spawn area. The Grid Index Type allows the Grid <em>Target Distribution Policy</em> to quickly access
                  all adjacent SpawnAreas, which is how BeatGrid finds targets to activate.
                </p>
              </div>
              <div className="article-subsection" ref={Ref_Spawning} id="target-lifecycle-Spawning">
                ...
                <p>
                  There are two spawning methods in BeatShot: <BSInlineEnum className={"ETargetSpawningPolicy"} valueName={"UpfrontOnly"} />{" "}
                  and <BSInlineEnum className={"ETargetSpawningPolicy"} valueName={"RuntimeOnly"} />.
                </p>
                <p>
                  Game modes using <BSInlineEnum className={"ETargetSpawningPolicy"} valueName={"UpfrontOnly"} /> spawn all targets inside{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"Init"} />. No other targets are spawned for the duration of
                  the game mode.
                </p>
                <p>
                  Game modes using <BSInlineEnum className={"ETargetSpawningPolicy"} valueName={"RuntimeOnly"} /> spawn their targets based
                  on beat thresholds being met by the audio analyzer, which triggers the game mode to call{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"OnAudioAnalyzerBeat"} />. I discuss this function more in
                  the Activation section, but for now I want to focus on what happens when any target is spawned, regardless of the Target
                  Spawning Policy.
                </p>
                <BSCodeBlock code={SpawnTarget} fontSize="0.65rem" />
                <p>
                  The location and scale of the target are retrieved from a previously found SpawnArea. I use SpawnActorDeferred so the game
                  mode config can be passed to the target before it&#39;s finished spawning.
                </p>
                <p>
                  <BSInlineFunction className={"ATarget"} functionName={"Init"} /> sets the Max Health attribute of the target and provides
                  all the information the target needs about the game mode. The Target Manager binds to the target&#39;s
                  OnTargetDamageEventOrTimeout delegate, which is broadcast when the health of the target is changed. This delegate is
                  discussed more in the Deactivation section.
                </p>
                <p>
                  The SpawnArea is assigned the target&#39;s global unique identifier, the target is added to the Target Manager&#39;s array
                  of managed targets, and{" "}
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"FlagSpawnAreaAsManaged"} /> is called. This lets
                  the Spawn Area Manager know that the SpawnArea now represents a target.
                </p>
                <p>
                  When a SpawnArea is flagged as <em>Managed</em>,{" "}
                  <BSInlineFunction className={"USpawnArea"} functionName={"GenerateOverlappingVertices"} /> is called. This function finds
                  all surrounding SpawnAreas that would cause an overlap if a target with the same radius is spawned inside that SpawnArea.
                  This is done by tracing a sphere centered at the location the target was spawned inside the SpawnArea. These traced
                  spheres are shown in Figure 2 as purple wireframes, while the red and green points are the center of individual
                  SpawnAreas.
                </p>
                <figure>
                  <div className="figure-border-container">
                ...
                <p>
                  The green points mean that it&#39;s currently safe to spawn a target in that SpawnArea, while the red points mean it&#39;s
                  not. Each time a new target occupies a given SpawnArea, these vertices generated and stored. They are then used when
                  finding subsequent target spawn locations, which is discussed in the Activation section.
                </p>
                <p>
                  The radius of the traced sphere should be big enough that it captures all SpawnAreas that would cause an overlap, but
                  small enough that it doesn&#39;t restrict the possible spawn locations to an unnecessary small amount. Since a target can
                  spawn anywhere inside a SpawnArea, some amount of error is introduced.
                </p>
                <p>
                  I ended up setting the traced sphere radius is equal to twice the radius of the target that was spawned in the spawn area
                  plus the larger of the width and height of the SpawnArea. This seemed to give me the best results when testing many
                  targets at once with varying scales, without restricting the number of spawn locations too much.
                </p>
              </div>
              <div className="article-subsection" ref={Ref_Activation} id="target-lifecycle-Activation">
                ...
                <p>
                  The chain of events leading to target activation begins when the game mode recieves input from the audio analyzer that the
                  beat threshold has been met, and calls{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"OnAudioAnalyzerBeat"} />. If using a song from a file, this
                  would occur exactly Spawn Beat Delay seconds before you actually hear the beat.
                </p>
                <BSCodeBlock code={OnAudioAnalyzerBeat} fontSize="0.65rem" />
                <p>
                  The TargetManager looks for any existing target(s) that can be activated, and tries to activate them according to the game
                  mode config using <BSInlineFunction className={"ATargetManager"} functionName={"HandleActivateExistingTargets"} />.
                </p>
                <p>
                  ...
                  ), <BSInlineFunction className={"ATargetManager"} functionName={"HandleRuntimeSpawnAndActivation"} /> is called.
                </p>
                ...
                <p>
                  ...
                  {" ("}
                  <BSInlineFunction className={"ATargetManager"} functionName={"FindNextTargetProperties"} />
                  {")"}.
                </p>
                <BSCodeBlock code={ActivateTarget} language={"c"} showLineNumbers={false} fontSize="0.65rem" />
                <p>
                  The only two SpawnArea objects that the Target Manager keeps track of are the PreviousSpawnArea and the CurrentSpawnArea.
                  The only place these two variables are changed is inside{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"FindNextTargetProperties"} />, and this function is always
                  called immediately after activating a target.
                </p>
                <BSCodeBlock code={FindNextTargetProperties} fontSize="0.65rem" />
                <p>
                  <BSInlineFunction className={"ATargetManager"} functionName={"GetNextTargetScale"} /> returns either a random target scale
                  between the Min and Max Target Scale or a dynamic target scale taken from a curve based on the number of recent targets
                  successfully destroyed.
                </p>
                <p>
                  The SpawnArea containing the next target location is found using{" "}
                  <BSInlineFunction className={"ATargetManager"} functionName={"GetNextSpawnArea"} />, which goes through several checks to
                  see if it can spawn at the origin or get a SpawnArea from the RL Component, but the main thing I want to focus on is when
                  it calls <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"GetValidSpawnLocations"} />. This
                  function is the heart of spawn location decision making.
                </p>
                <BSCodeBlock code={GetValidSpawnLocations} fontSize="0.65rem" />
                <p>
                  The game mode config&#39;s Target Distribution Policy dictates which series of functions are executed. Most distribution
                  policies begin considering all SpawnAreas as valid choices, rather than only finding SpawnAreas that aren&#39;t currently
                  activated or recent. This is because a smaller target may spawn before a larger target, and the cutout size of the sphere
                  it left will not be big enough to safely spawn the next larger target without overlapping the smaller one. This is also
                  why the target scale is found before finding the SpawnArea.
                </p>
                <p>
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"HandleFullRangeSpawnLocations"} /> removes all
                  SpawnAreas that fall outside of the current total spawn area. This only applies to game modes using{" "}
                  <BSInlineEnum className={"EBoundsScalingPolicy"} valueName={"Dynamic"} /> since the total spawn area is always the same
                  for game modes using <BSInlineEnum className={"EBoundsScalingPolicy"} valueName={"Static"} />. Referring to Figure 3, the
                  red boxes outside the the current spawn area (blue box) are the SpawnAreas this function removed from consideration for a
                  game mode using <BSInlineEnum className={"EBoundsScalingPolicy"} valueName={"Dynamic"} />.
                </p>
                <p>
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"HandleEdgeOnlySpawnLocations"} /> only adds the
                  points along the border of the current total spawn area.
                </p>
                <figure>
                  <div className="figure-border-container">
                    <Image src={image_SpawnMemory_Dynamic_FewRecent} alt="SpawnMemory_Dynamic_FewRecent" />
                    <figcaption>
                      <p className="figlabel">Figure 3: </p>
                      All Spawn Areas for a game mode using <BSInlineEnum className={"EBoundsScalingPolicy"} valueName={"Dynamic"} />, where
                      green boxes represent valid and red boxes represent invalid. The blue box represents the current total spawn area.
                    </figcaption>
                  </div>
                </figure>
                <p>
                  All Target Distribution Policies besides Grid call{" "}
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"RemoveOverlappingSpawnLocations"} />. This
                  function loops over all SpawnAreas that are flagged as <em>Activated</em> or <em>Recent</em> and adds their overlapping
                  vertices to a temporary array that is used to filter the spawn locations passed by reference to the function.
                </p>
                <BSCodeBlock code={RemovingOverlappingSpawnLocations} fontSize="0.65rem" />
                <p>
                  The overlapping vertices that are added are the same overlapping vertices that were generated when the target was spawned.
                  If the scale for the target to be spawned is greater than the scale that the overlapping points were generated with, a
                  temporary set is generated using the larger radius.
                </p>
                <p>
                  As soon as the target is activated by the Target Manager, the timelines that control the color and/or scale of the target
                  begin playing and a timer is set for the duration of its Max Lifespan. If the timer is allowed to expire, the target
                  damages itself using a <BSInlineCodeBlock code={"UGameplayEffect"} padding={"0"} /> that applies damage equal to the
              ...
              <div className="article-subsection" ref={Ref_Deactivation} id="target-lifecycle-Deactivation">
                ...
                <p>
                  ...
                  OnTargetDamageEventOrTimeout delegate introduced in the Spawning section is broadcast to the Target Manager.
                </p>
                <ul>
                  ...
                  <li>
                    <FontAwesomeIcon icon={faCrosshairs} className="li-icon" />
                    How far into the target&#39;s Max Lifespawn the target was destroyed. The value will be -1 if the target expired. This
                ...
                <p>
                  <BSInlineFunction className={"ATargetManager"} functionName={"OnTargetHealthChangedOrExpired"} /> is the function that all
                  targets broadcast the delegate to.
                </p>
                <p>
                  First, the Target Manager updates the consecutive targets hit and adjusts the dynamic scale factor. This value controls
                  the target scale if using <BSInlineEnum className={"EConsecutiveTargetScalePolicy"} valueName={"SkillBased"} /> and the
                  total spawn area if using <BSInlineEnum className={"EBoundsScalingPolicy"} valueName={"Dynamic"} />. The struct is then
                  forwarded to the game mode which updates the score and player HUD. The target is removed from the Target Manager&#39;s
                  managed target array if the Target Destruction Conditions permit. Any time a target is removed from this array,{" "}
                  <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"RemoveManagedFlagFromSpawnArea"} /> is also
                  called.
                </p>
                <p>Next, if the game mode uses the RL Component, the reward for the previous-current target location pair is updated.</p>
                <p>
                  Then, <BSInlineFunction className={"USpawnAreaManagerComponent"} functionName={"HandleRecentTargetRemoval"} /> is called.
                  This function removes the Activated flag from the SpawnArea and flags it as Recent. A timer is set based on the{" "}
                  <em>Recent Target Memory Policy</em> and the Recent flag is then removed when the timer expires. Upon removal of the
                  Recent flag, the overlapping vertices that were generated when the target was spawned are emptied.
                </p>
                <p>
                  If the game mode uses a Recent Target Memory Policy that keeps the footprint of recent targets too long, targets won&#39;t
                  have anywhere to be spawned. Figure 4 shows what it looks like moments before disaster (not really, the game simply
                  won&#39;t spawn a target until there&#39;s space).
                </p>
                <figure>
                  <div className="figure-border-container">
                    <Image src={image_SpawnMemory_Dynamic_ManyRecent} alt="SpawnMemory_Dynamic_ManyRecent" />
                    <figcaption>
                      <p className="figlabel">Figure 4: </p>
                      All Spawn Areas for a game mode using a Recent Target Memory Policy that keeps targets for 1.5 seconds after they are
                      deactivated.
                    </figcaption>
                  </div>
                </figure>
                <p>
                  Finally, the target then checks its Target Deactivation Conditions to see if it should deactivate. Deactivation conditions
                  are solely based on whether or not the target expired. If a deactivation condition was met,{" "}
                  <BSInlineFunction className={"ATarget"} functionName={"HandleDeactivationResponses"} /> is called.
                </p>
              ...
              <div className="article-subsection" ref={Ref_Destruction} id="target-lifecycle-Destruction">
                ...
                <p>
                  Just after checking deactivation conditions and broadcasting the OnTargetDamageEventOrTimeout delegate, the target checks
                  the Target Destruction Conditions to see if it needs to destroy itself. Since the target broadcast the delegate before
                  checking the Destruction Conditions, the Target Manager will have already removed it from its managed target array before
                  the target destroys itself.
                </p>
                <ol>
              ...
              <div className="article-subsection" ref={Ref_Conclusion} id="conclusion">
                ...
                <p>
                  Well, that was a lot. The system I described is by no means the only way to approach the problem, it&#39;s just the way
                  that made sense to me and worked. I hoped you learned something and weren&#39;t too overwhelmed.
                </p>
              </div>
