  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setAuth({ userID: data.userID, displayName: data?.displayName, accessToken: data.accessToken });
        setUsername("");
        setEmail("");
        setPassword("");
    ...
    <div className="flex-container-column padding-1rem">
      <div className="form-container">
        ...
        <form className="form" onSubmit={handleLogin}>
          <p className="fs-75 text-light text-center padding-top-05rem">Alternatively, use username or email:</p>
