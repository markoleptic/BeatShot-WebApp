...
import { authenticateSteamUser, createRelyingParty, fetchSteamUser, createSteamVerifyLinkAccountUrl } from "@/app/api/authfunctions";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (existingSteamUser && existingSteamUser.userID !== foundUser.userID) {
      return NextResponse.redirect(`${hostUrl as string}/redirect/?context=steamlinkedtodiffaccount`, { status: 302 });
    }

    await users.update({ userID: steamUser.steamid, displayName: steamUser.personaname }, { where: { userID: foundUser.userID } });
